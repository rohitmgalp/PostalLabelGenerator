...
# --- CACHED ALL-INDIA PINCODE CROSS-REFERENCE LOOKUP ENGINE ---
@st.cache_data
def lookup_pincode_details(pin_str):
    ...
    try:
        ...
        target = str(pin_str).strip()
        if '.' in target:
            target = target.split('.')[0]
            
        match = df[df['pincode'] == target]
        if not match.empty:
            row = match.iloc[0]
            return {
                "district": str(row.get("district", "")).upper().strip(),
                "statename": str(row.get("statename", "")).upper().strip()
            }
    except:
        pass
    return {"district": "", "statename": ""}
